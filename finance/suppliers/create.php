<?php
/**
 * finance/suppliers/create.php — Add a new supplier
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) { $errors[] = 'Session expired.'; }
        ...
        if (empty($errors)) {
            $pdo->prepare('INSERT INTO suppliers (name, contact_name, email, phone, address) VALUES (?,?,?,?,?)')->execute([$name, $contact_name ?: null, $email ?: null, $phone ?: null, $address ?: null]);
            $sid = (int)$pdo->lastInsertId();
            log_activity($uid, 'create_supplier', "Created supplier: $name.", 'supplier', $sid);
            flash_message('success', "Supplier '$name' added.");
...
<div class="content-card" style="max-width:620px;">
    ...
    <div class="content-card__body">
        <form method="post">
            ...
            <div style="display:flex;gap:var(--space-3);"><button type="submit" class="btn btn--primary">Add Supplier</button><a href="<?= e(app_base_url()) ?>/finance/suppliers/index.php" class="btn btn--secondary">Cancel</a></div>
        </form>
    </div>
</div>
</main>
