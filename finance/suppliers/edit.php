<?php
/**
 * finance/suppliers/edit.php — Edit supplier
 */
...
$sup_stmt = $pdo->prepare('SELECT * FROM suppliers WHERE id = ?');
$sup_stmt->execute([$sid]);
$sup = $sup_stmt->fetch();
if (!$sup) { flash_message('error', 'Supplier not found.'); header('Location: ' . app_base_url() . '/finance/suppliers/index.php'); exit; }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) { $errors[] = 'Session expired.'; }
        ...
        if (empty($errors)) {
            $pdo->prepare('UPDATE suppliers SET name=?,contact_name=?,email=?,phone=?,address=?,is_active=? WHERE id=?')->execute([$name, $contact_name ?: null, $email ?: null, $phone ?: null, $address ?: null, $is_active, $sid]);
            log_activity($uid, 'update_supplier', "Updated supplier: $name.", 'supplier', $sid);
            flash_message('success', 'Supplier updated.');
            header('Location: ' . app_base_url() . '/finance/suppliers/index.php'); exit;
        }
    }
}
$d = $_POST ?: $sup;
...
<div class="content-card" style="max-width:620px;">
    ...
    <div class="content-card__body">
        <form method="post">
            ...
            <div class="form-group"><label class="form-label">Company Name *</label><input class="form-input" name="name" value="<?= e($d['name']) ?>" required></div>
            <div class="form-group"><label class="form-label">Contact Person</label><input class="form-input" name="contact_name" value="<?= e($d['contact_name'] ?? '') ?>"></div>
            ...
            <div style="display:flex;gap:var(--space-3);"><button type="submit" class="btn btn--primary">Save Changes</button><a href="<?= e(app_base_url()) ?>/finance/suppliers/index.php" class="btn btn--secondary">Cancel</a></div>
        </form>
    </div>
</div>
</main>
